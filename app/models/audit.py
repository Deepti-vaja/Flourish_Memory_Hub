...
class AuditLog(Base):
    ...
    target_id: Mapped[uuid.UUID | None] = mapped_column(
        UUID(as_uuid=True), ForeignKey("knowledge_items.item_id", ondelete="RESTRICT"), nullable=True, index=True
    )
    ...
    target_item: Mapped["KnowledgeItem | None"] = relationship("KnowledgeItem", back_populates="audit_logs")
...
class AuditSequenceHead(Base):
    ...
    __table_args__ = (
        CheckConstraint("lock_key = 1", name="chk_singleton_head"),
    )
