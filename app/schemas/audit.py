...
class VerifyLedgerResponse(BaseDTOSchema):
    ...
    compromised: bool = Field(
        ..., description="True if any HMAC-SHA256 chain breaks or tampering is detected"
    )
    verified_records: int = Field(
        ..., description="Number of sequential records scanned and certified"
    )
    last_verified_seal: str | None = Field(
        default=None, description="HMAC-SHA256 seal of the most recent verified event"
    )
