...
class AssembleContextRequest(BaseDTOSchema):
    ...
    query: str | None = Field(
        default=None, max_length=2048, description="Optional search query string"
    )
    explicit_item_ids: list[UUID] | None = Field(
        default=None, description="Optional explicit list of UUID v4 IDs to include"
    )
    max_tokens: int = Field(
        default=2048,
        ge=256,
        le=32768,
        description="Maximum token ceiling for assembled context block",
    )
...
class AssembleContextResponse(BaseDTOSchema):
    ...
    assembled_prompt: str = Field(
        ..., description="Sanitized, structural <knowledge_citation> XML payload"
    )
    manifest: list[ContextManifestEntry] = Field(
        ...,
        validation_alias=AliasChoices("manifest", "lineage_manifest"),
        description="2-hop cryptographic lineage manifest",
    )
    tokens_consumed: int = Field(
        ...,
        validation_alias=AliasChoices("tokens_consumed", "tokens_used"),
        description="Total token budget consumed by system frame + items",
    )
    items_packed: int = Field(
        ...,
        validation_alias=AliasChoices("items_packed", "items_included"),
        description="Number of items packed into prompt",
    )
    items_omitted: int = Field(
        ...,
        validation_alias=AliasChoices("items_omitted", "items_omitted_budget"),
        description="Number of items omitted due to atomic token ceiling",
    )
