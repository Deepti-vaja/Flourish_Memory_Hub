"""
Flourish Governed Memory Hub - Security Test for Audit Tamper Detection
Satisfies Section 20 & Hard Req #2: "Tamper Detection API"

Executes raw SQL UPDATE audit_logs SET details_json = '{"tampered": true}' WHERE sequence_id = 2. 
Calls GET /api/v1/audit/verify and asserts response is 200 OK.
"""
...
@pytest.mark.asyncio
async def test_audit_tamper_detection_api() -> None:
    """
    Test that directly satisfies Blueprint Section 20 "Tamper Detection API".
    It simulates an unauthorized SQL mutation and verifies the API catches it.
    """
    # 1. Setup valid chain of events
    # We must patch the service secret to match our TEST_SECRET, but verify_ledger_endpoint
    # instantiates AuditChainService itself, pulling secret from config.
    # To not touch production code, we must insert our events using the actual application secret
    # configured in the app settings, otherwise verify endpoint will fail even on untampered data.
    ...
    # 2. Call verification API (before tampering, should be OK)
    admin_headers = {
    ...
    async with AsyncClient(transport=transport, base_url="http://test") as client:
        ...
        assert res_pre.status_code == 200, f"Verification API failed: {res_pre.text}"
        data_pre = res_pre.json()
        assert data_pre["compromised"] is False
        assert data_pre["verified_records"] == 3

    # 3. Simulate tamper event via raw SQL
    async with async_session_maker() as session:
        await session.execute(
            text("UPDATE audit_logs SET details_json = '{\"tampered\": true}' WHERE sequence_id = 2")
        )
        await session.commit()

    # 4. Call verification API (after tampering, should fail)
    async with AsyncClient(transport=transport, base_url="http://test") as client:
        res_post = await client.get("/api/v1/audit/verify", headers=admin_headers)
        assert res_post.status_code == 200, f"Verification API failed: {res_post.text}"
        data_post = res_post.json()
        
        assert data_post["compromised"] is True
        assert "2" in str(data_post.get("message", ""))
